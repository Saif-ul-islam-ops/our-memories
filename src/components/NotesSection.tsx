...
import { PenLine, Trash2, Loader2, X, Send } from "lucide-react";
import { addNote, fetchNotes, deleteNote } from "@/lib/firebase";
import { useScrollReveal } from "@/hooks/useScrollReveal";

export default function NotesSection() {
  const [notes, setNotes] = useState<Array<{ id: string; text: string; createdAt: any }>>([]);
  ...
  const load = async () => {
    setLoading(true);
    try { setNotes(await fetchNotes()); } catch {}
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  return (
    <section id="notes" ref={ref} className="py-24 px-4 opacity-0">
      <div className="container max-w-4xl">
        <h2 className="font-script text-4xl sm:text-5xl text-center text-romantic-deep mb-3" style={{ lineHeight: 1.15 }}>
          Love Notes
        </h2>
        <p className="text-center text-muted-foreground mb-10">Words from the heart</p>
        ...
        <div className="flex justify-center mb-10">
          ...
          >
            <PenLine className="w-5 h-5" /> Write a Note
          </button>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-center text-muted-foreground py-16">No notes yet. Write your first love note!</p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2">
            {notes.map((n, i) => (
              <div
                ...
                style={{ animation: `reveal-up 0.6s cubic-bezier(0.16,1,0.3,1) ${i * 80}ms forwards`, opacity: 0 }}
              >
                <p className="text-foreground/80 leading-relaxed whitespace-pre-wrap mb-4">{n.text}</p>
                <p className="text-xs text-muted-foreground">
                  {n.createdAt?.toDate?.()
                    ? new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(n.createdAt.toDate())
                    : "Just now"}
                </p>
                <button
      ...
      {/* Modal */}
      {modalOpen && (
        <div className="fixed inset-0 z-[100] bg-foreground/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="glass-strong rounded-3xl p-8 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-script text-2xl text-romantic-deep">Write a Note</h3>
              <button onClick={() => setModalOpen(false)} className="p-2 rounded-full hover:bg-muted transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <textarea
              value={text}
